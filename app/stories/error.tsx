...
import { RiErrorWarningLine, RiRefreshLine, RiHome4Line } from "@remixicon/react"
...
export default function StoriesError({ reset }: RouteErrorProps) {
  return (
    <main className="flex min-h-[calc(100svh-4rem)] flex-col items-center justify-center px-5 text-center">
      ...
      <p className="mt-2 max-w-xs text-sm leading-relaxed text-(--color-text-muted) md:mt-3 md:max-w-sm md:text-base">
        We couldn&apos;t load the stories right now. This is usually temporary
        - please try again.
      </p>
      ...
      <div className="mt-6 flex flex-col gap-3 sm:flex-row md:mt-8">
        ...
        >
          <Link href="/">
            <RiHome4Line className="mr-1.5 h-4 w-4 text-(--color-text-muted)" />
            Go Home
          </Link>
        </Button>
      </div>
